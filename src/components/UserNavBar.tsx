...
import { Bell, LogOut, Repeat, Loader2, ShieldCheck } from "lucide-react";
...
export const UserNavbar = () => {
  ...
  const { notifications, unreadCount, loading, refetchNotifications } =
    useNotifications();

  const { handleNotificationClick } = useMarkAsRead({
    onSuccess: () => {
    ...
    <>
      <nav className="w-full bg-white border-b border-gray-100 px-3 md:px-6 flex items-center justify-between shadow-sm sticky top-0 z-40 min-h-[64px]">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 md:gap-4 shrink-0">
          ...
          {/* 🔹 DYNAMIC MODE SWITCH BUTTON (Hidden for ADMIN) */}
          {!isAdmin && (
            <button
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <div
              className={`w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0 text-sm sm:text-base ${
                isAdmin ? "bg-amber-600" : "bg-emerald-950"
              }`}
            >
            ...
            <div className="hidden sm:flex flex-col">
              <span className="text-sm font-semibold text-emerald-950 leading-tight flex items-center gap-1">
