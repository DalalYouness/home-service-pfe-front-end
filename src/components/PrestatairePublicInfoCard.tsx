import { Briefcase, MapPin, Target, UserCheck } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-xl overflow-hidden transition-all duration-300">
      ...
      <div className="px-6 pb-6 pt-0 relative flex flex-col items-center text-center">
        ...
        {/* Section 3: REGISTRATION MESSAGE FOR VISITORS */}
        <div className="w-full mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
          <div
            className={`p-4 rounded-xl border ${theme.ctaBoxBg} text-center flex flex-col items-center gap-3 transition-colors duration-300`}
          >
            <div className="p-2.5 rounded-full bg-white dark:bg-slate-900 shadow-sm text-slate-700 dark:text-slate-200">
              <UserCheck size={20} />
            </div>

            <div className="space-y-1">
              <h4
                className={`text-xs sm:text-sm font-semibold ${theme.ctaText}`}
              >
                Vous souhaitez réserver ce prestataire ?
              </h4>
              <p className="text-slate-600 dark:text-slate-400 text-xs leading-relaxed max-w-md mx-auto">
                Pour effectuer une demande de service, vous devez être connecté
                en tant que client sur notre plateforme.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mt-1 w-full sm:w-auto">
              <button
                ...
                className={`w-full sm:w-auto py-2 px-5 rounded-lg ${theme.ctaBtnBg} text-white font-medium text-xs shadow-sm transition-all duration-200 hover:shadow active:scale-[0.98] cursor-pointer inline-flex items-center justify-center`}
              >
